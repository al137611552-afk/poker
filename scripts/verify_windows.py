...
MANUAL = [
    # —— 本轮改动，重点看这三条 ——
    ("UI", "**移动端**顶部五个标签单独占一行（不再挤在「第 N 手 · 盲注」后面），"
           "每个都好按"),
    ("UI", "五个标签**互斥**：点「场景训练」时牌桌收起来，当前标签高亮；"
           "**不会两个同时摊开**"),
    ("UI", "牌局进行中切到「场景训练」，再点回「牌桌」——**牌局原样还在、能接着打**；"
           "若切走时正轮到你行动，会显示一句「它会一直等着」"),
...
def main() -> int:
    ...
    for group, text in MANUAL:
        print(f"  [ ] [{group}] {text}")
    print("-" * 70)
    print(f"\n⚠ **脚本全绿不等于验完了** —— 上面那 {len(MANUAL)} 条要人来看。")
