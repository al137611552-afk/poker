...
- 根节点已经在底池里的钱算「赢得到的」，不算我方投入——「这个局面对我值多少」的自然读法。
  **别把这条当成「跟求解器一致」**（这里以前就是这么写的，2026-08-21 实测推翻了）：
  TexasSolver 报的是**净收益**，根节点底池里属于自己的那一半不算赢得到的。
  两边差的恒是那一份：`我们的 EV = 求解器 EV + 底池的一半 + 从根节点起自己投进去的钱`。
  接求解器直给的 EV（`backend.solve_evs`）时必须翻译，有慢测逐动作互证；
